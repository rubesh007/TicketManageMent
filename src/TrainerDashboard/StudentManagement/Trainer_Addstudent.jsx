...
import { FiEdit, FiEdit2, FiPlus, FiSearch, FiTrash2, FiX } from "react-icons/fi";
...
const Trainer_Addstudent = () => {
    ...
    const handleStudenstActivity = ( e ) => {
        const { name, value } = e.target;
        setActivityStudent( { ...activityStudent, [ name ]: value } );
    };

    const handleActivitySubmits = ( e ) => {

        const { studentStatus, courseCompleted, reviewCompleted,
            studentColorCode, studentAttitude, comments } = activityStudent;
        ...
        if ( studentStatus &&
            ...
            studentAttitude &&
            comments !== '' ) {
            const activityUpdate = [ ...activityStore, activityStudent ];
            setActivityStore( activityUpdate );
            localStorage.setItem( 'StudentActivity', JSON.stringify( activityUpdate ) );
            toast.success( "Successfully Save" );
            setActivity( !activity );
        } else {
            toast.error( 'Fill All Activity' );
        }
    };
                                    ...
                                    <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                                        <thead className={ `${ darkMode ? 'bg-gray-700' : 'bg-gray-50' }` }>
                                            <tr>
                                                ...
                                                <th className="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider">
                                                    <span className={ darkMode ? 'text-gray-300' : 'text-gray-600' }>Activity</span>
                                                </th>
                        ...
                        { activity ?
                            <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
                                <div className={ `rounded-xl shadow-lg w-full max-w-4xl ${ darkMode ? 'bg-gray-800' : 'bg-white' }` }>
                                    ...
                                    {/* Student Activity form start point */ }
                                    <form onSubmit={ handleActivitySubmits } className="p-6 space-y-4">
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                            ...
                                            <div className="space-y-1">
                                                ...
                                                <select
                                                    id="studentStatus"
                                                    name="studentStatus"
                                                    onChange={ handleStudenstActivity }
                                                    required
                                                    className={ `w-full px-3 py-2 rounded-md border ${ darkMode
                                                        ? 'bg-gray-700 border-gray-600 text-white'
                                                        : 'bg-white border-gray-300 text-gray-900'
                                                ...
                                                >
                                                    ...
                                                    <option value='Move to Placements'>Move to Placements</option>
                                                    <option value='Training in Progress'>Training in Progress</option>
                                                    <option value='Already Placed'>Already Placed</option>
                                                    <option value='On Break Informed'>On Break Informed</option>
                                                    <option value='Taken Combo Courses'>Taken Combo Courses</option>
                                                    <option value='No Response'>No Response</option>
                                                    <option value='Placement not Required'>Placement not Required</option>
                                                    <option value='Complaint or Issue'>Complaint / Issue</option>
                                                    <option value='mock or Project Pending'>mock / Project Pending</option>
                                                    <option value='Shifted to Another Trainer'>Shifted to Another Trainer</option>
                                                    <option value='Onboard_Interviews'>Onboard_Interviews</option>
                                                </select>
                                            </div>
                                            ...
                                            <div className="space-y-1">
                                                ...
                                                >
                                                    <option value=''>--Select--</option>
                                                    <option value='Sincere and Hardworking'>Sincere and Hardworking</option>
                                                    <option value='Casual Still Does Tasks'>Casual Still Does Tasks</option>
                                                    <option value='Lethargic Skipped Many Tasks'>Lethargic Skipped Many Tasks</option>
                                                </select>
                                            </div>
                                            {/* trainer Comments*/ }
                                            <div className="space-y-1">
...
export default Trainer_Addstudent;
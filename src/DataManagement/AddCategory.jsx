...
const AddCategory = () => {
    ...
    useEffect( () => {
        const stored = localStorage.getItem( "CategoryDetails" );
        if ( stored ) {
    ...
    const handleSubmit = ( e ) => {
        ...
        setCategory( updated );
        localStorage.setItem( "CategoryDetails", JSON.stringify( updated ) );
        setFormData( { categoryname: "", status: "" } );
        setEditIndex( null );
        setShowModal( false );
    };
                        ...
                        <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-6">
                            <div>
                                ...
                                <p className={ `mt-1 ${ darkMode ? 'text-gray-400' : 'text-gray-600' }` }>
                                    Create and manage training category
                                </p>
                            </div>
                        ...
                        { category.length === 0 ? (
                            <div className={ `rounded-xl p-8 text-center ${ darkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200' } border shadow-sm` }>
                                ...
                                <p className={ `mt-2 ${ darkMode ? 'text-gray-500' : 'text-gray-600' }` }>
                                    You haven't added any category yet. Get started by adding one!
                                </p>
                                <button
                                    onClick={ handleAdd }
                            ...
                            <div className={ `rounded-xl shadow-sm overflow-hidden ${ darkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200' } border` }>
                                ...
                                { filteredCategory.length === 0 && (
                                    <div className="py-12 text-center">
                                        ...
                                        <h3 className={ `text-lg font-medium ${ darkMode ? 'text-gray-300' : 'text-gray-700' }` }>No category found</h3>
                            ...
                            <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
                                <div className={ `rounded-xl shadow-lg w-full max-w-md ${ darkMode ? 'bg-gray-800' : 'bg-white' }` }>
                                    ...
                                    <form onSubmit={ handleSubmit } className="p-6">
                                        <div className="mb-5">
                                            <label htmlFor="categoryname" className={ `block mb-2 text-sm font-medium ${ darkMode ? 'text-gray-300' : 'text-gray-700' }` }>
                                                Category Name
                                            </label>
                                            <input
                                                type="text"

...
const AddBatch = () => {
    ...
    const [ formData, setFormData ] = useState( { batchname: "", status: "" } );
    ...
    useEffect( () => {
        const stored = localStorage.getItem( "BatchDetails" );
        if ( stored ) {
    ...
    const handleSubmit = ( e ) => {
        ...
        setBatches( updated );
        localStorage.setItem( "BatchDetails", JSON.stringify( updated ) );
        setFormData( { batchname: "", status: "" } );
        setEditIndex( null );
        setShowModal( false );
    };
                        ...
                        { batches.length === 0 ? (
                            <div className={ `rounded-xl p-8 text-center ${ darkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200' } border shadow-sm` }>
                                ...
                                <p className={ `mt-2 ${ darkMode ? 'text-gray-500' : 'text-gray-600' }` }>
                                    You haven't added any batch yet. Get started by adding one!
                                </p>
                                <button
                                    onClick={ handleAdd }
                            ...
                            <div className={ `rounded-xl shadow-sm overflow-hidden ${ darkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200' } border` }>
                                    ...
                                    <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                                        <thead className={ `${ darkMode ? 'bg-gray-700' : 'bg-gray-50' }` }>
                                            <tr>
                                                ...
                                                <th className="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider">
                                                    <span className={ darkMode ? 'text-gray-300' : 'text-gray-600' }>Batch Name</span>
                                                </th>
                                                <th className="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider">
                                ...
                                { filteredBatches.length === 0 && (
                                    <div className="py-12 text-center">
                                        ...
                                        <h3 className={ `text-lg font-medium ${ darkMode ? 'text-gray-300' : 'text-gray-700' }` }>No batches found</h3>
                            ...
                            <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
                                <div className={ `rounded-xl shadow-lg w-full max-w-md ${ darkMode ? 'bg-gray-800' : 'bg-white' }` }>
                                    ...
                                    <form onSubmit={ handleSubmit } className="p-6">
                                        <div className="mb-5">
                                            <label htmlFor="batchname" className={ `block mb-2 text-sm font-medium ${ darkMode ? 'text-gray-300' : 'text-gray-700' }` }>
                                                Batch Name
                                            </label>
                                            <input
                                                type="text"

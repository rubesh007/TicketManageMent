...
export const Student_Project = () => {
  ...
  useEffect( () => {
    ...
    const trainerData = localStorage.getItem( "TeacherDetails" );
    if ( trainerData ) {
      setTrainers( JSON.parse( trainerData ) );
    }
  ...
  const userProjects = useMemo( () => {
    ...
    return dataStore.filter( item => item.userId === currentUser.id );
  ...
  const currentUsers = userProjects.slice( start, end );
  const totalPages = Math.ceil( userProjects.length / usersPerPage );
  ...
  const handleProjectSubmit = ( e ) => {
    e.preventDefault();
    const { date, name, course, email, projecttitle, trainer, status } = project;

    if ( date && name && course && projecttitle && email && status && trainer ) {
      const projectData = {
        ...project,
  ...
  const handleEdit = ( id ) => {
    ...
    if ( selectedIndex !== -1 ) {
      setProject( dataStore[ selectedIndex ] );
  ...
  }, [ currentUsers, search, userProjects ] );
